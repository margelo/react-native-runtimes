...
<Callout type="info" title="Fire-and-forget vs. await">
  Use `schedule(fn).on(runtimeName)(...args)` when the caller only needs to
  enqueue work and observe progress through shared state. Use
  `call(fn).on(runtimeName)(...args)` when the caller needs the return value.
</Callout>
