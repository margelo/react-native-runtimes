...
<Callout type="warn" title="Keep UI imports out of index.background.ts">
  ...
  runtime functions, hydrate stores, start app-lifetime queues, and install
...
Use `schedule(fn).on(runtimeName)(...args)` for fire-and-forget jobs that
report progress through shared state — including native callers that must
queue work before JavaScript has a convenient caller. For request/response
work, prefer the function directive or `call(fn).on(runtimeName)(...args)`.
